...
Build a streaming pipeline that consumes NYC taxi events from Kafka, processes them with Spark Structured Streaming, and lands data into an Apache Iceberg lakehouse using the medallion architecture (bronze → silver → gold).

Group's custom scenario: Modify produce.py so that messages after row 500 include an additional field airport_fee with value 1.75 (add it to the msg dict before sending). Your bronze layer must accept both message shapes without failing. In REPORT.md, show how both versions are stored in the bronze table and how you verified no rows were lost.
